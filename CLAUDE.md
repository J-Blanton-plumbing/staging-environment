...
25. **Article V2 shares `/knowledge-hub/[slug]` with V1, so its CSS must stay code-split (Brief 190).** `cms_articles.template` (`'article'` | `'article-v2'`, unknown → V1) picks the template; `cms_articles.v2` (JSONB) holds the V2 fields. Both travel in the version content and reach the live row only on Publish (the `terms` contract, gotcha 24); the bottom "Save Article" PUT writes neither. **Any CSS the route's server graph reaches, or that a client component it imports eagerly reaches, is linked on EVERY article** — a plain `import './article-v2.css'` in `ArticleV2Template.tsx` put the 20 KB V2 stylesheet on all ~812 V1 articles. The only importer is `ArticleV2Styles.tsx`, loaded with `next/dynamic` from the client component `ArticleV2Client.tsx`; that puts the CSS in an async chunk Next links (PreloadCss) only when a V2 article renders. Do not "simplify" that import. Also since Brief 190: article **Preview** shows the draft's content (`getArticlePreview`, session-gated) — before, it silently showed the live row. Since Brief 192 the V2 phone is by REGION: an article whose office is in Central Ohio (`officeRegion` in offices.ts: state OH) uses Global Settings' **Central Ohio phone** (`global_settings.central_ohio_phone_*`, read by `getRegionalPhonesCached` — deliberately NOT in the `GlobalSettings` object, which the root layout serializes onto every page); anything else uses the main phone. Brief 190's per-office phone field is gone. Content components (Promise list / Service rows / Feature cards) are structured `v2.components` rendered by `v2-blocks/V2Blocks.tsx` at `[[component:<name>]]` markers — never HTML classes in the body. The WhatConverts DOM fallback (`fromDom()`) accepts a number only from an anchor it saw holding the DEFAULT number earlier on the same path (Brief 192 Track D), and still skips `[data-wc-ignore]`.
