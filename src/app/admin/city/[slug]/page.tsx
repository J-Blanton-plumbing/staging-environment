...
function CoverageAreaCityFields({
  ...
  form: FormState;
  setField: (k: keyof Omit<FormState, 'faqs' | 'templateType'>, v: string) => void;
  missing: string[];
}) {
  return (
    <>
...
export default function AdminCityPage() {
  ...
  function setField(key: keyof Omit<FormState, 'faqs' | 'templateType'>, value: string) {
    setForm(f => {
  ...
  function handleSwitched(newTemplate: string, missing: string[]) {
    ...
    fetch(`/api/cms/city/${slug}`)
      .then(async r => {
        const data = await r.json();
        setForm({
          templateType: data.templateType ?? newTemplate,
          heroImage: data.heroImage ?? '',
          heroHeadingLine1: data.heroHeadingLine1 ?? '',
          heroHeadingLine2: data.heroHeadingLine2 ?? '',
          heroCallout: data.heroCallout ?? '',
  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif' }}>
      <AdminPageHeader
        ...
        availableTemplates={['coverage-area', 'local-office']}
        ...
        templateName={form.templateType === 'local-office' ? 'Local Office City' : 'Coverage Area City'}
        previewBaseUrl={`/${slug}`}
      />
