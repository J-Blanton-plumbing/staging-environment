...
interface FormState {
  ...
  customer_service_body: string;
  billing_questions_label: string;
  billing_questions_body: string;
  plumbing_issue_label: string;
  plumbing_issue_body: string;
  meta_title: string;
...
const EMPTY: FormState = {
  ...
  customer_service_label: '', customer_service_body: '',
  billing_questions_label: '', billing_questions_body: '',
  plumbing_issue_label: '', plumbing_issue_body: '',
...
export default function HelpAndSupportAdminPage() {
    ...
    fetch('/api/cms/main/help-and-support')
      ...
      .then(data => {
        setForm({
          ...
          customer_service_label: data.customer_service_label ?? '',
          customer_service_body: data.customer_service_body ?? '',
          billing_questions_label: data.billing_questions_label ?? '',
          billing_questions_body: data.billing_questions_body ?? '',
          plumbing_issue_label: data.plumbing_issue_label ?? '',
          plumbing_issue_body: data.plumbing_issue_body ?? '',
  ...
  async function handleSave() {
    setStatus('saving');
    try {
      const res = await fetch('/api/cms/main/help-and-support', {
        method: 'PATCH',
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem' }}>
        ...
        {(
          [
            ...
            ['Have a Plumbing Issue?', 'plumbing_issue'],
          ] as [string, string][]
        ).map(([title, key]) => (
          <div key={key} style={sec}>
            <h2 style={{ fontWeight: 700, marginBottom: '1rem' }}>{title}</h2>
            <label style={lbl}>Label</label>
            <input style={s} value={form[`${key}_label` as keyof FormState]} onChange={e => set(`${key}_label` as keyof FormState, e.target.value)} />
