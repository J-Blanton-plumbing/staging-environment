...
import type { DrainContent } from '@/lib/content/drain';
...
async function getContent(cmsOverride?: ServiceCmsContent): Promise<DrainContent> {
  try {
    const cms = cmsOverride ?? await getServiceCmsContent('drain');
    if (cms) {
      ...
      return {
        ...
        subcategories: {
          ...
          items: subcategories.map((sub, i) => ({
            label: sub.label,
            href: sub.href,
            image: DRAIN.subcategories.items[i]?.image ?? '',
        ...
        heroImage: DRAIN.heroImage,
        fImage: DRAIN.fImage,
        f3Image: DRAIN.f3Image,
...
export default async function DrainPage() {
  ...
  const servicePreview = await getServicePreview('drain');
    ...
    <>
      {previewDraft && (
        <PreviewBanner
          ...
          editorUrl="/admin/drain"
          liveUrl="/services/drain"
