 ...
 *  • `heroImage` is set for EVERY Ohio area — its own map hero from
 *    `public/images/columbus-heroes/`, one file per area (Brief 173; Brief 172
 *    shipped the same thing behind an eight-slug pilot gate, now removed). This
 *    is the one place these pages diverge from `elgin.ts`, which still takes the
 *    generic CloudFront fallback.
...
import type { OhioArea } from './ohio-areas';
import { OHIO_STATE, getOhioArea } from './ohio-areas';
import { COLUMBUS_HERO_PREFIX } from './shared';

/**
 * The map hero for one area — the OSM map with its boundary traced in Carmine,
 * generated by `scripts/columbus-heroes/build-heroes.mjs`.
 *
 * Brief 172 shipped this behind a `HERO_PILOT_SLUGS` allowlist so Marketing
 * could sign the treatment off on eight pages first. They did; Brief 173
 * removed the gate, so every Ohio area now gets its own map. There is no
 * per-area opt-out — an area with no image on disk would 404 its hero rather
 * than fall back, which is why Track A of that brief regenerates all 138 and the
 * run reports any `nodata`.
 *
 * The path is derived from the slug rather than tabulated: neighborhood files
 * drop the `columbus-` prefix (`columbus-short-north` →
 * `hero_columbus_short-north.webp`), municipalities use the slug as-is. Verified
 * against all 138 rows of `columbus-hero-manifest.csv`, which the generator
 * rewrites from each full run.
 *
 * The LEADING SLASH is load-bearing: `resolveCityImage()` passes a root-relative
 * path through untouched but turns a bare filename into a CloudFront URL, and
 * these files exist only on this origin.
 *
 * `columbus` never reaches this function — `getOhioTemplateContent()` returns
 * `undefined` for it so `./columbus.ts` is never shadowed — so its identical
 * `heroImage` is set in that file instead.
 */
function heroImageFor(area: OhioArea): string {
  const core = area.kind === 'neighborhood' ? area.slug.replace(/^columbus-/, '') : area.slug;
  return `${COLUMBUS_HERO_PREFIX}hero_columbus_${core}.webp`;
}
...
export function getOhioTemplateContent(slug: string): CoverageAreaContent | undefined {
  ...
  return {
    ...
    // `meta` intentionally omitted — see the docblock. `heroImage` is the area's
    // own map (Brief 173), so no Ohio page takes the CloudFront pipes fallback
    // any more. A CMS `hero_image` value still wins over this in `[city]/page.tsx`.
    heroImage: heroImageFor(area),
  };
}
